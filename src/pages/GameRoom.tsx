...
function CardImage({ name, className = '' }: { name: string; className?: string }) {
  ...
  return <img src={src} alt={name} className={`rounded object-cover ${className}`} onError={() => setErr(true)} />;
}
...
export default function GameRoom() {
  ...
  const loadState = useCallback(async () => {
    ...
    const gs = initGame(
      ds.players.player1.name, ds.players.player1.picks,
      ds.players.player2.name, ds.players.player2.picks,
    );
